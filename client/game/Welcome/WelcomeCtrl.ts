...
export default class WelcomeCtrl extends BaseRoundCtrl<IRoundDataStore> {
  ...
  public LoginPlayer() {
    ...
    SapienServerCom.SaveData(
      ...
      SapienServerCom.BASE_REST_URL + "auth"
    )
      .then((r: { team: TeamModel; user: UserModel; token: string }) => {
        console.log("RETURNED BY AUTH ENDPOINT: ", r);
        localStorage.setItem("RH_USER", JSON.stringify(r.user));
        localStorage.setItem("rhjwt", r.token);

        this.dataStore.ApplicationState.CurrentUser = this.dataStore.ApplicationState.CurrentUser = Object.assign(
          new UserModel(),
          r.user
        );

        if (
          r.user.Role == RoleName.FACILITATOR ||
          r.user.Role == RoleName.ADMIN
        ) {
          this.component.props.history.push(
            "/facilitator/base/" + r.team.GameId
          );
        } else {
          this.dataStore.ApplicationState.CurrentTeam = this.dataStore.ApplicationState.CurrentTeam = Object.assign(
            new TeamModel(),
            r.team
          );
