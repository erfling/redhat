...
> {
  ...
  public createOrEditGame(game?: GameModel) {
    this.dataStore.ApplicationState.ModalObject =
      Object.assign(new GameModel(), game) || new GameModel();
    if (!this.dataStore.ApplicationState.ModalObject.DatePlayed)
      this.dataStore.ApplicationState.ModalObject.DatePlayed = new Date().toLocaleDateString();
  ...
  public saveGame(game: GameModel) {
    ...
    return SapienServerCom.SaveData(
      game,
      SapienServerCom.BASE_REST_URL + "games"
    )
      .then((r) => {
        if (game._id) {
          ...
          this.dataStore.Admin.FilteredGames = this.dataStore.Admin.FilteredGames.map(
            (g) => {
              return g._id == game._id ? Object.assign(game, r) : g;
            }
          );
          ...
          this.dataStore.Admin.FilteredGames = this.dataStore.Admin.FilteredGames.concat(
            Object.assign(new GameModel(), r)
          );
        }
  ...
  public saveTeam(team: TeamModel, game: GameModel) {
    ...
    return SapienServerCom.SaveData(
      team,
      SapienServerCom.BASE_REST_URL + "games/team"
    )
      .then((r) => {
  ...
  public filterUsersByGame(game: GameModel): void {
    ...
    this.dataStore.Admin.AvailablePlayers = this.dataStore.Admin.Users.filter(
      (u) => u._id && userIds.indexOf(u._id) == -1
    ).map((u, i) => {
      return {
  ...
  public saveUser(user: UserModel) {
    ...
    return SapienServerCom.SaveData(
      ...
      this.dataStore.ApplicationState.FormIsSubmitting = false;
      this.dataStore.ApplicationState.ModalTarget.Players = this.dataStore.ApplicationState.ModalTarget.Players.filter(
        (p) => p._id != null
      ).concat(returnedUser);
      this.closeModal();
      return returnedUser;
  ...
  public DeleteTeam(team: TeamModel) {
    ...
    return SapienServerCom.SaveData(
      team,
      SapienServerCom.BASE_REST_URL + "games/team/delete"
    )
      .then((r) => {
  ...
  public DeleteGame(game: GameModel) {
    return SapienServerCom.DeleteData(
      ...
      .then((r) => {
        this.dataStore.Admin.Games = this.dataStore.Admin.Games.filter(
          (g) => g._id != game._id
        );
        this.dataStore.Admin.FilteredGames = this.dataStore.Admin.FilteredGames.filter(
          (g) => g._id != game._id
        );
