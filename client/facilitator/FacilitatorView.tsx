...
export default class FacilitatorView extends BaseComponent<
  any,
  {
...
> {
  ...
  handleClick = (e, titleProps: { active: boolean; index: number }) => {
    ...
    if (active) {
      this.controller.dataStore.FacilitatorState.AccordionIdx = this.controller.dataStore.FacilitatorState.AccordionIdx.filter(
        (num) => num != index
      );
    } else {
      this.controller.dataStore.FacilitatorState.AccordionIdx = this.controller.dataStore.FacilitatorState.AccordionIdx.concat(
        index
      );
    }
  ...
  getUserIsComplete(
    mapping: FacilitationRoundResponseMapping,
    user: UserModel
  ) {
    ...
    if (
      ...
      if (user.Job == JobName.MANAGER) {
        let completedRatings = mapping.RatingsByManager.filter(
          (r) => r.IsComplete
        );
        if (completedRatings.length == mapping.RatingsByManager.length)
          return <Icon name="checkmark" color="green" />;
      }
      //has this play rated the manager
      else {
        let thisPlayersRatingOfManager = mapping.RatingsOfManager.filter(
          (r) => r.IsComplete && r.UserId && r.UserId == user._id
        );
        if (thisPlayersRatingOfManager.length)
          return <Icon name="checkmark" color="green" />;
      }
    }
  ...
  render() {
    ...
    const renderResponse = (q: QuestionModel) => {
      if (
        !q.Response ||
        !q.Response.Answer ||
        !(q.Response.Answer as SliderValueObj[]).length
      ) {
        return (
          <>
            <p>N/A</p>
      ...
      const renderAnwser = (a: SliderValueObj) => (
        <div className="facilitator-responses">
          ...
          <p>
            ...
            {a.data}
            {a.unit && a.unit}
            {a.idealValue && a.idealValue == a.data && (
              <Icon name="check" style={{ marginLeft: "5px" }} color="green" />
            )}
          </p>
          {a.idealValue && (
            <>
          ...
          {a.idealValue && a.idealValue != a.label && (
            <>
              <p>
                <strong>Ideal Response: </strong>
                {a.idealValue}
                <Icon name="cancel" style={{ marginLeft: "5px" }} color="red" />
              </p>
            </>
          )}
        </div>
        ...
        return (
          <div className="facilitator-responses">
            {orderBy(q.Response.Answer, "data").map((a: SliderValueObj, i) => (
              <>
                <p>
                  {`${i + 1}. ${a.label}`} (Ideal Position:{" "}
                  {Number(a.idealValue) + 1})
                </p>
              </>
            ))}
          </div>
      ...
      const renderRatingAnswers = (q) => {
        return (
          <>
            {orderBy(q.Response.Answer, "label").map(
              (answer: SliderValueObj[], i) =>
                answer.map((a) => (
                  <p>
                    {a.label}: {a.data}
                  </p>
      ...
      switch (q.Type) {
        case QuestionType.MULTIPLE_CHOICE:
          if (q.SubRoundLabel == "3A" || q.SubRoundLabel == "2A") {
            return answer
              .filter((a) => a.data == true || a.data == true.toString())
              .map((a) => renderAnwser(a));
          } else if (q.SubRoundLabel == "1B") {
            return answer
              .filter((a) => a.data == true || a.data == true.toString())
              .map((a) => render1BAnwser(a));
          } else {
        ...
        case QuestionType.NUMBER:
          if (q.SubRoundLabel == "2A") {
            return answer.map((a) => render2ASlider(a));
    ...
    return (
      ...
      >
        <Column mobile={16} tablet={16} computer={12} largeScreen={10}>
          <Header as="h1">Facilitator Dashboard</Header>
          {this.state.FacilitatorState.RoundResponseMappings &&
            this.state.FacilitatorState.RoundResponseMappings.length > 0 && (
              <Segment>
                <Header as="h2">
                  Round: (
                  {
                    this.state.FacilitatorState.RoundResponseMappings[0]
                      .RoundName
                  }{" "}
                  -{" "}
                  {
                ...
                <Header as="h2">
                  PIN:{" "}
                  {this.state.FacilitatorState.Game &&
                    this.state.FacilitatorState.Game.GamePIN}
                </Header>
              </Segment>
            )}
          <Segment>
            <Button
              ...
              onClick={() =>
                window.open(
                  "/facilitator/slides/" + this.state.FacilitatorState.Game._id,
                  "_blank"
                )
              }
            >
            ...
            >
              <Modal.Header>
                Scores through Round{" "}
                {this.state.FacilitatorState.RoundResponseMappings &&
                  this.state.FacilitatorState.RoundResponseMappings.length &&
                  this.state.FacilitatorState.RoundResponseMappings[0]
                    .SubRoundLabel}
              </Modal.Header>
              <Modal.Content>
                <FacilitatorScoreDisplay
                  Stepped={false}
                  SubRoundScores={
                    this.state.FacilitatorState.FacilitatorScores.SubRoundScores
                  }
                  RoundScores={
                    this.state.FacilitatorState.FacilitatorScores.RoundScores
                  }
                  CumulativeScores={
                    this.state.FacilitatorState.FacilitatorScores
                      .CumulativeScores
                  }
          ...
          {this.state.FacilitatorState.RoundResponseMappings && (
            <Row>
              <Accordion
                ...
                style={{ width: "100%" }}
                exclusive={false}
                defaultActiveIndex={this.state.FacilitatorState.AccordionIdx}
              >
                {this.state.FacilitatorState.RoundResponseMappings.map(
                  (t, i) => (
                    <>
                      <Accordion.Title
                        active={
                          this.state.FacilitatorState.AccordionIdx.indexOf(i) !=
                          -1
                        }
                        index={i}
                      ...
                      >
                        ...
                        <Button
                          onClick={(e) => {
                            e.preventDefault();
                            this.controller.getTeamResponses(
                              t,
                              this.state.FacilitatorState.Game.CurrentRound
                                .ChildRound
                            );
        ...
        {this.state.FacilitatorState.ShowRolesModal &&
          this.state.FacilitatorState.ModalTeam && (
            <TeamJobsModal
              Team={this.state.FacilitatorState.ModalTeam}
              SaveFunction={this.controller.UpdateTeamJobs.bind(
                this.controller
              )}
              CloseFunction={() => {
              ...
              ValidationFunc={this.controller.validateTeamJobs.bind(
                this.controller
              )}
              Submitting={
                this.controller.dataStore.FacilitatorState.ModalTeam.IsSaving
              }
            />
          )}
        ...
        {this.state.FacilitatorState.SelectedTeamMapping && (
          <Modal
            ...
            open={
              this.state.FacilitatorState.SelectedTeamMapping &&
              this.state.FacilitatorState.SelectedTeamMapping.Questions.length >
                0
            }
          >
            <Modal.Header>
              {this.state.FacilitatorState.SelectedTeamMapping.TeamName}'s
              Responses
            </Modal.Header>
            <Modal.Content>
              <header>
                {this.state.FacilitatorState.ModalRoundFilter.rounds.map(
                  (r) => (
                    <Button
                      circular
                      inverted={
                        this.state.FacilitatorState.ModalRoundFilter.value != r
                      }
                      color="blue"
                      onClick={(e) => {
                        this.controller.dataStore.FacilitatorState.ModalRoundFilter.value = r;
                        this.setState({
                          FacilitatorState: this.controller.dataStore
                            .FacilitatorState,
                        });
                      }}
                    >
                ...
                <div className="ratings-switch">
                  <Checkbox
                    label="Show Ratings"
                    toggle
                    defaultChecked={
                      this.controller.dataStore.FacilitatorState
                        .ModalRoundFilter.showRatings
                    }
                    onChange={() => {
                      this.controller.dataStore.FacilitatorState.ModalRoundFilter.showRatings = !this
                        .controller.dataStore.FacilitatorState.ModalRoundFilter
                        .showRatings;
                      this.setState({
                        FacilitatorState: {
              ...
              {!this.state.FacilitatorState.ModalRoundFilter.showRatings &&
                this.state.FacilitatorState.SelectedTeamMapping.Questions.filter(
                  (q) =>
                    q.RatingMarker != RatingType.IC_RATING &&
                    q.RatingMarker != RatingType.MANAGER_RATING &&
                    q.RatingMarker != RatingType.TEAM_RATING &&
                    ratingRounds.indexOf(q.SubRoundLabel) == -1 &&
                    q.SubRoundLabel &&
                    q.SubRoundLabel.toUpperCase() ==
                      this.state.FacilitatorState.ModalRoundFilter.value.toUpperCase()
                ).map((q) => (
                  <>
                    <h2>{q.Text}</h2>
                    {renderResponse(q)}
              ...
              {!this.state.FacilitatorState.ModalRoundFilter.showRatings &&
                (this.state.FacilitatorState.ModalRoundFilter.value.toUpperCase() ==
                  ...
                  this.state.FacilitatorState.ModalRoundFilter.value.toUpperCase() ==
                    "4B") &&
                this.state.FacilitatorState.SelectedTeamMapping.Questions.filter(
                  (q) => q.RatingMarker == RatingType.TEAM_RATING
                ).map((q) => (
                  <>
                    <h2>
                      Ratings Submitted by{" "}
                      {this.state.FacilitatorState.SelectedTeamMapping.TeamName}{" "}
                      in Round{" "}
                      {this.state.FacilitatorState.ModalRoundFilter.value.toUpperCase()}
                    </h2>
              ...
              {this.state.FacilitatorState.ModalRoundFilter.showRatings &&
                this.state.FacilitatorState.SelectedTeamMapping.Questions.filter(
                  (q) => q.RatingMarker == RatingType.MANAGER_RATING
                ).map((q) => {
                  let ratings = this.controller.getGroupedRatings(
                    q,
                    "SubmitterLabel"
                  );
                      ...
                      {q.Response && q.Response.Answer && (
                        <ul className="facilitator-ratings-display">
                          {Object.keys(ratings).map((k, i) => {
                            let subRatings = this.controller.getGroupedChildren(
                              ratings[k],
                              "TargetUserLabel",
                              this.state.FacilitatorState.ModalRoundFilter.value
                            );
                            return (
                              <li key={i}>
                                <h3>Rated By {k}</h3>
