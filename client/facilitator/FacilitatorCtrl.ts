...
export default class FacilitatorCtrl extends BaseClientCtrl<{
  ...
  public onClickChangeSlide(forwardOrBack: 1 | -1): void {
    ...
    if (lookup) {
      ...
      if (
        ...
        if (
          this.dataStore.FacilitatorState.CurrentLookup.RoundScoreIdx >
          this.dataStore.FacilitatorState.Game.Teams.length
        ) {
          hold = false;
        }
      ...
      this.dataStore.FacilitatorState.SlideNumber = hold
        ? this.dataStore.FacilitatorState.SlideNumber
        : number;
      this.dataStore.FacilitatorState.CurrentLookup = hold
        ? this.dataStore.FacilitatorState.CurrentLookup
        : lookup;
  ...
  public getRoundInfo() {
    ...
    return SapienServerCom.GetData(
      ...
      if (!this.dataStore.FacilitatorState.AccordionIdx)
        this.dataStore.FacilitatorState.AccordionIdx = (rcl as FacilitationRoundResponseMapping[]).map(
          (lookup, i) => i
        );

      if (slideMapping.SlideNumber.toString() in SpecialSlides) {
        slideMapping = Object.assign(
          slideMapping,
          SpecialSlides[this.dataStore.FacilitatorState.SlideNumber.toString()]
        );
        if (
          slideMapping.SlideFeedback &&
          !this.dataStore.FacilitatorState.Game.CurrentRound.SlideFeedback
        ) {
      ...
      if (!this.dataStore.FacilitatorState.ModalRoundFilter.value)
        this.dataStore.FacilitatorState.ModalRoundFilter.value =
          rcl[0].SubRoundLabel;
  ...
  public getGame(id: string) {
    ...
    return SapienServerCom.GetData(
      ...
      console.log(
        "HEY",
        this.dataStore.FacilitatorState,
        g.CurrentRound,
        this.dataStore.FacilitatorState.CurrentLookup
      );
  ...
  public gotToSlide() {
    GameCtrl.GetInstance().dataStore.ApplicationState.CurrentGame.CurrentRound.SlideNumber = DataStore.ApplicationState.CurrentGame.CurrentRound.SlideNumber = this.dataStore.ApplicationState.CurrentGame.CurrentRound.SlideNumber = this
  ...
  public getChartingScores() {
    ...
    SapienServerCom.GetData(null, null, url).then(srs => {
      let r = srs.map((sr: SubRoundScore) =>
        Object.assign(
          new SubRoundScore(),
          Object.assign(sr, { NormalizedScore: sr.NormalizedScore * 100 })
        )
      );
      console.log(r);
      ...
      this.dataStore.FacilitatorState.SlideScores.RoundScores = this.getRoundRanking(
        r
      );
      this.dataStore.FacilitatorState.SlideScores.CumulativeScores = this.getCumulativeTeamScores(
        r
      );
    });
  }
  ...
  getRoundRanking(scores: SubRoundScore[], allScores: boolean = false) {
    ...
    let accumulatedScores = Object.keys(groupedTeamScores).map(k => {
      ...
      accumulatedScore.NormalizedScore =
        groupedTeamScores[k].reduce((totalScore, srs) => {
    ...
    if (this.dataStore.FacilitatorState.Game.Teams.length > 2) {
      this.dataStore.FacilitatorState.CurrentLookup.RoundScoreIdx = Math.max(
        3,
        this.dataStore.FacilitatorState.Game.Teams.length - 2
      );
    } else {
      this.dataStore.FacilitatorState.CurrentLookup.RoundScoreIdx = this.dataStore.FacilitatorState.Game.Teams.length;
    }

    if (this.dataStore.FacilitatorState.CurrentLookup.SkipRoundScore)
      this.dataStore.FacilitatorState.CurrentLookup.RoundScoreIdx = -1;
  ...
  getCumulativeTeamScores(scores: SubRoundScore[], allScores: boolean = false) {
    console.log("GETTING CUMULATIVE", scores, allScores);
    ...
    if (this.dataStore.FacilitatorState.Game.Teams.length > 2) {
      this.dataStore.FacilitatorState.CurrentLookup.CumulativeScoreIdx = Math.max(
        3,
        this.dataStore.FacilitatorState.Game.Teams.length - 2
      );
    } else {
  ...
  public async getFacilitatorScores() {
    ...
    return SapienServerCom.GetData(null, null, url).then(srs => {
      ...
      this.dataStore.FacilitatorState.FacilitatorScores.RoundScores = this.getRoundRanking(
        r
      );
      this.dataStore.FacilitatorState.FacilitatorScores.CumulativeScores = this.getCumulativeTeamScores(
        r
      );
      this.component.setState({
        FacilitatorState: this.dataStore.FacilitatorState
  ...
  public async UpdateTeamJobs(team: FacilitationRoundResponseMapping) {
    this.dataStore.ApplicationState.ValidationErrors = this.validateTeamJobs(
      team,
      this.dataStore.FacilitatorState.Game
    );
  ...
  public validateTeamJobs(
    team: FacilitationRoundResponseMapping,
    game: GameModel = this.dataStore.FacilitatorState.Game
  ): (boolean | string)[] {
    const hasJob = (
      team: FacilitationRoundResponseMapping,
      jobName: JobName,
  ...
  public getGroupedRatings(q: QuestionModel, prop: string) {
    if (q.Response && q.Response.Answer) {
      let flats = (q.Response.Answer as SliderValueObj[]).reduce(
        (flattened, a) => {
  ...
  public getGroupedChildren(
    answers: SliderValueObj[],
    prop: string,
    round: string
  ) {
