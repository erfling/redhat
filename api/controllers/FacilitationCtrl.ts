...
class FacilitationCtrl {
  ...
  public async getRoundChangeLookups(req: Request, res: Response) {
    try {
      //get rounds and subrounds so we can only return lookups from active rounds and subrouds
      const roundIds: string[] = await monRoundModel
        .find({ IsActive: true })
        .then((rounds) => rounds.map((round) => round._id.toString()));

      const subRoundIds: string[] = await monSubRoundModel
        .find({ IsActive: true })
        .then((subRounds) =>
          subRounds.map((subRound) => subRound._id.toString())
        );

      const lookups: RoundChangeLookup[] = await monRoundChangeLookupModel
        .find({
          IsActive: true,
        }) /*.po
        pulate("Round").populate("SubRound")*/
        .then((lookups) =>
          lookups
            ? lookups.map((l) =>
      ...
      res.json(
        lookups.filter(
          (lookup) =>
            roundIds.indexOf(lookup.RoundId) !== -1 &&
            subRoundIds.indexOf(lookup.SubRoundId) !== -1
        )
      );
    } catch (err) {
  ...
  public async GetTeamsFinished(req: Request, res: Response) {
    try {
      ...
      let subRound: SubRoundModel = await monSubRoundModel
        .findById(game.CurrentRound.SubRoundId)
        .populate("Questions")
        .then((sr) =>
      ...
      let round: RoundModel = await monRoundModel
        .findById(game.CurrentRound.RoundId)
        .then((sr) =>
      ...
      game.Teams.forEach((t) => {
        ...
        let teamResponses = responses.filter(
          (r) => r.TeamId == t._id.toString()
        );
  ...
  public routes() {
    ...
    this.router.get("/lookups", this.getRoundChangeLookups.bind(this));
    this.router.get("/round-status/:gameid", this.GetTeamsFinished.bind(this));
