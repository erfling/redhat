...
import FacilitationCtrl, {
  monRoundChangeLookupModel,
} from "./controllers/FacilitationCtrl";
...
class SubRoundNormalizationInfo {
  ...
  public static RoundWeights: SubRoundNormalizationRule[] = [
    { RoundId: "1", SubRoundId: "a", Label: "1a", weight: 0.2 },
    { RoundId: "1", SubRoundId: "b", Label: "1b", weight: 0.8 },
  ];
}
...
export class AppServer {
  ...
  private static getServer(): http.Server | https.Server {
    try {
      if (AppServer._isProd()) {
        console.log("ENV VARS ARE:", process.env, "see");
        var privateKey = fs
          .readFileSync(
        ...
        var certificate = fs
          .readFileSync(
            "/sapien/certificates/planetsapien.com/fullchain.pem",
            "utf8"
          )
          .toString();
        if (!privateKey || !certificate)
          throw new Error("no cert or key found");
  ...
  public static StartServer() {
    ...
    AppServer.WebServer.on("error", AppServer.onError).on(
      "listening",
      AppServer.onListening
    );
  }
  ...
  private static onListening(): void {
    ...
    if (AppServer._isProd()) {
      MONGO_URI =
        "mongodb://mbreeden:F5aJyDx4F9Ly@localhost:27017/red-hat?authSource=admin"; //F5aJyDx4F9Ly
      ...
      async (req, res, next) => {
        ...
        if (game) {
          if (!game.CurrentRound || !game.CurrentRound.ParentRound) {
            ...
            !req.query ||
            req.query.force ||
            !req.query.ParentRound ||
            !req.query.ChildRound ||
            req.query.ParentRound.toUpperCase() !=
              game.CurrentRound.ParentRound.toUpperCase() ||
            req.query.ChildRound.toUpperCase() !=
              game.CurrentRound.ChildRound.toUpperCase()
          ) {
    ...
    AppServer.app
      ...
      .use(
        "/sapien/api/gameplay",
        /*Passport.authenticate('jwt', { session: false }),*/ GamePlayCtrl
      )
      .use(
        "/sapien/api/facilitator",
        /*Passport.authenticate('jwt', { session: false }),*/ FacilitationCtrl
      )

      .post(
        "/sapien/api/facilitation/round/:gameid",
        Passport.authenticate("jwt", { session: false }),
        async (req, res) => {
          try {
            ...
            const mapping: RoundChangeMapping = await GamePlayUtilities.HandleRoundChange(
              Object.assign(new RoundChangeMapping(), req.body),
              game
            );
...
/*
            ...
            if (mapping.ShowFeedback || mapping.SlideFeedback) {
              ...
              var subRounds: SubRoundModel[] = await monSubRoundModel
                .find({ RoundId: mapping.RoundId })
                .populate("Questions")
                .then((srs) =>
                  srs.map((sr) =>
              ...
              for (let j = 0; j < subRounds.length; j++) {
                ...
                for (let i = 0; i < game.Teams.length; i++) {
                  ...
                  let srs = GamePlayUtilities.HandleScores(
                    questions,
                    responses,
                    game,
                    t,
                  ...
                  var oldScore: SubRoundScore = await monSubRoundScoreModel
                    .findOne({ TeamId: t._id, SubRoundId: subRound._id })
                    .then((sr) =>
                      sr
                        ? Object.assign(new SubRoundScore(), sr.toJSON())
                        : null
                    );
                  if (oldScore && oldScore.BonusPoints)
                    srs.NormalizedScore += oldScore.BonusPoints;
                  var savedSubRoundScore: SubRoundScore = await monSubRoundScoreModel
                    .findOneAndUpdate(
                      { TeamId: t._id, SubRoundId: subRound._id },
                      srs,
  ...
  private static async _setRoundAndSubroundOrder() {
    try {
      //do rounds
      const rounds: RoundModel[] = await monRoundModel
        .find()
        .populate("SubRounds")
        .populate({
          path: "PrevRound",
        ...
        .then((rs) =>
          rs ? rs.map((r) => Object.assign(new RoundModel(), r.toJSON())) : null
        );
        ...
        for (let j = 0; j < r.SubRounds.length; j++) {
          ...
          if (j == 0) {
            if (!r.PrevRound) {
              ...
              sr.PrevSubRound = prevSubrounds
                ? prevSubrounds[prevSubrounds.length - 1]._id
                : null;
            }
            sr.NextSubRound = r.SubRounds[j + 1]
              ? r.SubRounds[j + 1]._id
              : null;
          } else if (j == r.SubRounds.length - 1) {
            sr.PrevSubRound = r.SubRounds[j - 1]
              ? r.SubRounds[j - 1]._id
              : null;
          ...
          for (let n = 0; n < sr.Questions.length; n++) {
            ...
            let savedQuestion = await monQModel.findByIdAndUpdate(
              q._id,
              { SubRoundId: sr._id },
              { new: true }
            );
          ...
          if (!savedSr)
            throw new Error(
              JSON.stringify({ message: "Couldn't save subround: ", sr })
            );
        }
      }
    } catch (err) {
  ...
  public static async setMessageRoundLabels() {
    const srs = await monSubRoundModel
      .find()
      .then((srs) =>
    ...
    for (let i = 0; i < srs.length; i++) {
      ...
      let populatedMessages: MessageModel[] = await monMessageModel
